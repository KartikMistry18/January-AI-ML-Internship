...
if selected == "Clean Data":
    ...
    st.markdown(
        "Upload one or multiple CSV files to preprocess and clean your files quickly and stress free."
    )
        ...
        if len(dataframes) > 1:
            ...
            if merge:
                ...
                keep_first_header_only = st.selectbox(
                    "Keep only the header (first row) of the first file", ["Yes", "No"]
                )
                remove_duplicate_rows = st.selectbox(
                    "Remove duplicate rows", ["No", "Yes"]
                )
                ...
                try:
                    ...
                    st.error(
                        "Please make sure columns match in all files. If you don't want them to match, select 'No' in the first option."
                    )
                    st.stop()
    ...
    st.markdown("")
    st.markdown(
        "<p style='text-align: center'><a href='https://github.com/shantanugondane/CleanSweep'>Github</a></p>",
