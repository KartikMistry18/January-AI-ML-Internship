...
data= pd.read_csv("C:/Users/karti/OneDrive/Desktop/Winter intern/INTERNPE/diabetes (1).csv")
...
if selected == "Predict Diabetes":
    ...
    DPF = st.number_input("Diabetes Predigree Function",min_value=0.000, max_value=100.000,step=0.001, format="%0.3f")
    ...
    pred = loaded_model.predict([[Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, bmi, DPF, age]])
...
if selected == "Contribute to Dataset":
    ...
    if st.button("Submit"):
        to_add={"Pregnancies":[preg], "Glucose":[gluc], "Blood Pressure":[bp], "Skin Thickness":[skinth], "Insulin":[ins], "BMI":[BMI], "DiabetesPedigreeFunction":[dpf], "Age":[Age], "Outcome":[out]}
        to_add=pd.DataFrame(to_add)
        to_add.to_csv("C:/Users/karti/OneDrive/Desktop/Winter intern/INTERNPE/diabetes (1).csv", mode = 'a', header=False, index=False)
